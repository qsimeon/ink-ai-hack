...
import { STYLE_PRESETS, DEFAULT_STYLE_PRESET, buildPrompt } from './services/stylePresets';
...
function App() {
  ...
  // 3D CAD viewer state (STL/OBJ) — null means modal is closed
  const [viewer3dState, setViewer3dState] = useState<{
    geometry: THREE.BufferGeometry;
    fileName: string;
  } | null>(null);
  const cad3dFileInputRef = useRef<HTMLInputElement>(null);
  ...
  // Handle CAD file upload — parse SVG/DXF and add a CadSketchElement to the canvas
  const handleCadUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const isD = file.name.toLowerCase().endsWith('.dxf');
      const { primitives, viewBox } = isD ? parseDxf(text) : parseSvg(text);

      if (primitives.length === 0) {
        console.warn('[CAD] No geometry found in', file.name);
        return;
      }

      const { strokes, timings, totalDuration } = geometryToStrokes(primitives);

      const canvasX = window.innerWidth / 2;
      const canvasY = window.innerHeight / 2;
      const element = createCadSketchElement(canvasX, canvasY, {
        strokes,
        strokeTimings: timings,
        totalDuration,
        viewBox,
  ...
  // Handle 3D CAD file upload (STL/OBJ) — open viewer modal
  const handle3dUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      ...
      if (file.name.toLowerCase().endsWith('.stl')) {
        ...
        geometry = loadStlGeometry(buffer);
      } else {
        const text = await file.text();
        geometry = loadObjGeometry(text);
      }
      setViewer3dState({ geometry, fileName: file.name });
    } catch (err) {
      console.error('[CAD 3D] Failed to load file:', err);
    }
    e.target.value = '';
  }, []);

  // Called by CadViewer3D "Sketch This View" — converts projected edges to animation
  const handleSketch3d = useCallback((primitives: GeometryPrimitive[]) => {
    if (!viewer3dState) return;
    // 3D-projected paths are already in canvas pixels — reduce jitter so
    // straight lines look straight, not wobbly. Default 1.5px is too much.
    const { strokes, timings, totalDuration } = geometryToStrokes(primitives, { jitter: 0.4 });
    const canvasX = window.innerWidth / 2;
    const canvasY = window.innerHeight / 2;
    const element = createCadSketchElement(canvasX, canvasY, {
      strokes,
      ...
      sourceFileName: viewer3dState.fileName,
    });
    element.animationStartTime = Date.now();
    const latest = currentNoteRef.current;
    setCurrentNote({ ...latest, elements: [...latest.elements, element] });
    ...
    <div className="app">
      ...
      <header className="toolbar">
        ...
        <div className="toolbar-section tool-buttons">
          ...
          {/* CAD file upload — accepts .svg and .dxf */}
          <input
            ref={cadFileInputRef}
            type="file"
            accept=".svg,.dxf"
          ...
          <button
            ...
            title="Import CAD drawing (.svg or .dxf) and animate it as a hand-drawn sketch"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* 3D CAD file upload — accepts .stl and .obj */}
          <input
            ref={cad3dFileInputRef}
            type="file"
            accept=".stl,.obj"
            style={{ display: 'none' }}
          ...
          />
          <button
            onClick={() => cad3dFileInputRef.current?.click()}
            title="Import 3D model (.stl or .obj) — view in isometric and sketch to canvas"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
